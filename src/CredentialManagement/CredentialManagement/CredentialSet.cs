...
namespace CredentialManagement
{
	public class CredentialSet : List<Credential>, IDisposable
	{
		...
		private void LoadInternal()
		{
			...
			List<NativeMethods.CriticalCredentialHandle> list = array.Select((IntPtr ptrCred) => new NativeMethods.CriticalCredentialHandle(ptrCred)).ToList();
			IEnumerable<Credential> collection = list.Select((NativeMethods.CriticalCredentialHandle handle) => handle.GetCredential()).Select(delegate(NativeMethods.CREDENTIAL nativeCredential)
			{
